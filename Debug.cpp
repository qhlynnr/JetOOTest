...
int run() {
    ...
    TH1F h2("h2", "Jet A_{J} of OO Sample", hvar.nbin, hvar.xmin, hvar.xmax);
    h2.SetDirectory(0);
    ...
    TH1F h_midntrk("h_midntrk", "Jet A_{J} of OO Sample (200 < ntrk < 400)", hvar.nbin, hvar.xmin, hvar.xmax);
    h_midntrk.SetDirectory(0);

    for(int subfolderIndex = 0; subfolderIndex < ForestSubfolder.size(); subfolderIndex++) {
        string subfolder = ForestSubfolder.at(subfolderIndex);
        ...
        for(int i = 1; i < 120; i++){
            ...
            for (Long64_t entrynum = 0; entrynum < nEntries; entrynum++){
                ...
                cout << "Event num: " << entrynum 
                    ...
                    << endl;
                    if (nTrkNew > 400) {
                        cout << "Filling histogram for nTrk > 400" << endl;
                        h_highntrk.Fill(A_J); // Fill the histogram for events with nTrk > 600
                    } else if (nTrkNew < 400 && nTrkNew > 200) {
                        cout << "Filling histogram for 200 < nTrk < 400" << endl;
                        h_midntrk.Fill(A_J); // Fill the histogram for events with nTrk < 600 and > 200
                    } else {
                        cout << "Filling histogram for nTrk < 200" << endl;
                        h_lowntrk.Fill(A_J); // Fill the histogram for events with nTrk
                    }
                    cout << "Filling histogram for A_J: " << A_J << endl;
                    //h2.Fill(A_J); // Fill the histogram for the selected events with online 14OR
            }
